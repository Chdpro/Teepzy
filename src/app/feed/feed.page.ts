...
export class FeedPage implements OnInit {
  ...
  addFavorite(postId) {
    ...
    this.subscription = this.contactService.addFavorite(favoris).subscribe(
      (res) => {
        //  this.socket.emit('notification', 'notification');
        //   console.log(res)
        this.listPosts.find((c, index) =>
          c["_id"] == postId ? (c["favorite"] = true) : null
        );
  ...
  removeFavorite(postId) {
    ...
    this.subscription = this.contactService.removeFavorite(favoris).subscribe(
      (res) => {
        //  console.log(res)
        this.listPosts.find((c, index) =>
          c["_id"] == postId ? (c["favorite"] = false) : null
        );
        this.contactService.setLocalData(
          CACHE_KEYS.FEEDS_CHECK,
          this.listPosts
