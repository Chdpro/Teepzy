...
export class ChatPage implements OnInit, AfterViewChecked {
  ...
  @ViewChild("clickHoverMenuTrigger", null)
  ...
  markMessagesRead(roomId, userId) {
    ...
    this.subscription = this.contactService.markReadMessages(room).subscribe(
      (res) => {
        console.log(res);
      },
      (error) => {
  ...
  replyto(msg) {
    ...
    msg.messageRepliedId
      ? (this.repliedMessage.messageId = msg.messageRepliedId)
  ...
  checkUserInMyCircle(userId, connectedUserId) {
    if (userId == connectedUserId) {
      ...
      this.subscription = this.contactService.checkInMyCircle(check).subscribe(
        (res) => {
          //   console.log(res)
          res["data"] == false
      ...
      this.contactService.checkInMyCircle(check).subscribe(
        (res) => {
          //   console.log(res)
          res["data"] == false
            ? (this.isInMyCircle = false)
  ...
  sendMessage() {
    ...
    this.message.createdAt =
      currentTime.toLocaleDateString() + "T" + this.getCurrentTime();
    let t =
      currentTime.toLocaleDateString().split("/").reverse().join("") +
      this.getCurrentTime().split(":").join("");
  ...
  getMessages(id) {
    ...
    this.contactService.ChatRoomMessages(obj).subscribe(
      (res) => {
        //  console.log(res)
        this.loading = false;
  ...
  getChatRoomUserInitiator(id) {
    this.authService.myInfos(id).subscribe(
      (res) => {
        //  console.log(res)
