...
export class ProfilePage implements OnInit {
  ...
  constructor(
    ...
    this.route.queryParams.subscribe((params) => {
      if (this.router.getCurrentNavigation().extras.state) {
        this.listProducts =
          this.router.getCurrentNavigation().extras.state.listProducts;
        this.listProjects =
          this.router.getCurrentNavigation().extras.state.listProjects;
      }
    });
  }
  ...
  subscriptions() {
    ...
    this.subscriptionFavorites = this.dataPass
      .getFavorite()
      .subscribe((list) => {
        //console.log(list)
        if (list.length > 0) {
    ...
    this.subscriptionMyTeepz = this.dataPass.getPosts().subscribe((list) => {
      //console.log(list)
      if (!list) this.listTeepz = [];
      else this.listTeepz = list;
    });
  }
  ...
  goToDetailMesTeepz(idTeepz) {
    this.router.navigate([
      ...
      {
        idTeepz: idTeepz,
        previousUrl: "mesTeepz",
        previousBackUrl: this.previousUrl,
      },
    ]);
  }
  goToDetailTeepz(idTeepz) {
    this.router.navigate([
      "/detail-feed",
      { idTeepz: idTeepz, previousUrl: "mesFavorite" },
    ]);
  }
    ...
    if (userId === this.user["_id"]) {
      if (this.previousUrl == "feed") {
        this.router.navigate([
          ...
          {
            previousUrl: "feed",
            idUser: this.route.snapshot.paramMap.get("userId"),
          },
        ]);
      } else {
