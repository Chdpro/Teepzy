...
export class Tab3Page implements OnInit {
  ...
  removeRoom(room) {
    ...
    if (this.userId === room["userId"]) {
      this.subscription = this.contactService
        .removeRoomByInitiator(room._id)
        .subscribe(
          (res) => {
            //console.log(res);
      ...
      this.subscription = this.contactService
        .removeRoomByConnectedUser(room._id)
        .subscribe(
          (res) => {
            //console.log(res);
            this.showToast("Conversation Supprimée");
  ...
  getChatRooms() {
    this.loading = true;
    let conversations = [];
    this.subscription = this.contactService.mChatRooms(this.userId).subscribe(
      (res) => {
        let r = res["data"];
        ...
        for (const room of r) {
          if (room["lastMessage"].length > 0) {
            ...
            let lastMessage = [
              {
                timeStamp:
                  room.createdAt.slice(0, 9).split("-").join("") +
                  room.createdAt.slice(12, 16).split(":").join(""),
              },
            ];
            conversations.push({
              connectedUsers: room.connectedUsers,
